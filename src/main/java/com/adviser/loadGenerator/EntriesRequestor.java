...
public class EntriesRequestor {
	...
	final ArrayBlockingQueue<Runnable> queue = new ArrayBlockingQueue<Runnable>(BROWSERS*BrowserRequestLimit);
	
    ThreadPoolExecutor threadPool = new ThreadPoolExecutor(BrowserRequestLimit, BROWSERS*BrowserRequestLimit,
    ...
    public EntriesRequestor() {
    	...
    	cm.setMaxTotal((BrowserRequestLimit*BROWSERS)*2);
    	// Increase default max connection per route to 20
    	cm.setDefaultMaxPerRoute(BrowserRequestLimit*BROWSERS*2);
    }
    ...
    private class ProcessEntries {
    	...
    	private class ProcessEntry implements Runnable {
    		...
    		private HttpRequestBase method(Request request) throws UnsupportedEncodingException {
    			if (request.getMethod().equals("GET")) return new HttpGet(request.getUrl());
    			if (request.getMethod().equals("POST")) {
    				...
    				//LOGGER.warn("method post is not implemented: {}", request.getUrl());
    				return post;
    			}
    			LOGGER.warn("method cannot work with method:{}",request.getMethod());
    			return null;
    		}
			public void run() {
				int requests = 0;
				...
				while (true) {
					...
					if (entryIdx >= entries.size()) {
						LOGGER.info("Done: {} {} {}", Thread.currentThread().getId(), "" + requests + ":" + bytes);
						break;
					}
					...
					try {
						...
						final HttpRequestBase request = method(entry.getRequest());
						...
						if (response.getStatusLine().getStatusCode() != 200) {
							LOGGER.error("httpResponse:Status: {} {}", response.getStatusLine().getStatusCode(), entry.getRequest().getUrl());
						}
						...
						LOGGER.info("httpRequest:UNKNOWNHost:"+entry.getRequest().getUrl());
					} catch (Throwable e) {
						LOGGER.error("httpRequest:"+entry.getRequest().getUrl(), e);
					}

					
					++requests;
					
					//LOGGER.info("Process: {} {}", Thread.currentThread().getId(), entry.getRequest().getUrl());
				}
			}
    		
    	}
    	public ProcessEntries(List<Entry> entries) {
    		this.entries = entries;
    		this.processed.set(0);
    		for(int i = 0; i < BrowserRequestLimit; ++i) {
    			threadPool.execute(new ProcessEntry());
    		}
    	}
     }
    
	public void request(List<Entry> entries) {
